...
def extract_information(name, data):
    with open(name, 'w') as file_write:
        ...
        file_write.write('------------------------\n')
        file_write.write('Shift km: ' + str(data['SPECTRA_FP4'].header[8]) +
                         '!!! this shift is already applied on altitude of limb spectra.\n')
...
def extract_and_save_spectra(name, data, altitude):
    ...
    nb_spectra = len(data[0]) - 1  # 1st: wavenumber

    for i in range(nb_spectra):
        with open(name + '/limb_' + str(int(altitude.field(0)[i])) + 'km.spe', "ab") as file_write:
            # the first 3 lines is adapted to our retrieval code, you can remove these lines
...
def main():
    ...
    for i, fits_name in enumerate(list_fits):
        ...
        with fits.open(fits_name) as fits_info:

            extract_information(directory_root + 'information.txt', fits_info)

            data_fp3 = fits_info['SPECTRA_FP3'].data
            data_altitude_fp3 = fits_info['ALTITUDE_FP3'].data
            extract_and_save_spectra(directory_root + 'spectra_fp3', data_fp3, data_altitude_fp3)

            data_fp4 = fits_info['SPECTRA_FP4'].data
            data_altitude_fp4 = fits_info['ALTITUDE_FP4'].data
            extract_and_save_spectra(directory_root + 'spectra_fp4', data_fp4, data_altitude_fp4)

            extract_and_save_profile(directory_root + directory_input + '/thermal_profile',
                                     fits_info['THERMAL_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/c2h2_profile', fits_info['C2H2_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/c2h4_profile', fits_info['C2H4_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/c2h6_profile', fits_info['C2H6_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/c3h8_profile', fits_info['C3H8_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/c3h4_profile', fits_info['C3H4_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/c4h2_profile', fits_info['C4H2_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/c6h6_profile', fits_info['C6H6_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/hcn_profile', fits_info['HCN_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/hc3n_profile', fits_info['HC3N_PROFILE'].data)
            extract_and_save_profile(directory_root + directory_input + '/co2_profile', fits_info['CO2_PROFILE'].data)
